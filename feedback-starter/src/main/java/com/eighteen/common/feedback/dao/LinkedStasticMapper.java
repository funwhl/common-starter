...
public interface LinkedStasticMapper {
    @Select("<script>" +
            "select top ${count}  imei,iimei,channel, coid, ncoid, wifi,ip, firstLinkTime activeTime, ua, androidId, oaid,mid ,ROW_NUMBER() OVER(ORDER BY firstLinkTime asc) AS r from ${tableName}  with(nolock) " +
            "where 1=1 " +
