package com.eighteen.common.spring.boot.autoconfigure.mybatis.feedback.dao;
...
public interface FeedBackMapper {
    @Select("<script> select top ${num} a.*, click.* from ActiveLogger a " +
            " INNER JOIN KuaiShouClickLog click" +
            " ON a.imeimd5 = click.imei " +
            "and a.activetime >= click.click_time and a.status = 0 </script>"
    )
...
//    int updateFeedbackStatus(@Param("imeis") List<String> imeis, @Param("status") Integer status);
    ...
    @Update("<script>" +
            ...
            "</foreach>" +
            "</script>")
    int updateFeedbackStatus(@Param("data") List<String> data, @Param("status") Integer status, Integer coid, Integer ncoid);
